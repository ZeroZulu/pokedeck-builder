...
export default function App() {
  ...
  const doSearch = useCallback(async (pg = 1) => {
    ...
    try {
      ...
      if (!controller.signal.aborted) {
        setError("Failed to fetch cards. You may be rate-limited — try again in a moment.");
        setCards([]);
      }
    }
          ...
          <div ref={gridRef} style={{ flex: 1, overflow: "auto", padding: isMobile ? 10 : 14, display: "grid", gridTemplateColumns: isMobile ? "repeat(auto-fill,minmax(100px,1fr))" : isTablet ? "repeat(auto-fill,minmax(110px,1fr))" : "repeat(auto-fill,minmax(120px,1fr))", gap: isMobile ? 8 : 10, alignContent: "start", WebkitOverflowScrolling: "touch" }}>
            {loading ? (
              ...
              <div style={{ gridColumn: "1/-1", textAlign: "center", padding: 30, color: "#fca5a5" }}>
                ...
                <div style={{ fontSize: 12, color: "#64748b", lineHeight: 1.6, maxWidth: 300, margin: "0 auto" }}>
                  The free API allows ~30 requests/minute. Wait a moment and try again, or get a free API key at
                  <a href="https://dev.pokemontcg.io" target="_blank" rel="noreferrer" style={{ color: "#22d3ee", marginLeft: 4 }}>dev.pokemontcg.io</a>
                  {" "}for 20× higher limits.
                </div>
                <button onClick={() => doSearch(1)} style={{ marginTop: 12, padding: "8px 20px", borderRadius: 8, background: "rgba(34,211,238,.12)", color: "#22d3ee", border: "1px solid rgba(34,211,238,.25)", cursor: "pointer", fontSize: 13, fontWeight: 600, fontFamily: "inherit" }}>🔄 Retry</button>
              </div>
            ) : cards.length === 0 ? (
